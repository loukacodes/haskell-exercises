...
simplify (RationalNumber a b) = RationalNumber (a `div` cd) (b `div` cd) where cd = gcd a b
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = RationalNumber x 1 where x = if a > 0 then 1 else if a == 0 then 0 else -1

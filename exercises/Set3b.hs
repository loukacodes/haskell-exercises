...
sumsOf' :: [Int] -> Int -> [Int]
sumsOf' [] _ = []
sumsOf' (x:xs) initialNum = (x + initialNum) : sumsOf' xs (x + initialNum)

-- how it runs: 
-- e.g sumOf [a,b,c] => (a+0):(b+a+0):(c+b+a+0):[] => [a, a+b, a+b+c]
...
maybeMap f (x:xs) = case (f x) of
    Nothing -> maybeMap f xs -- just continue with the next item in list
    Just v -> v : maybeMap f xs -- append the return value and continue with the next item in list

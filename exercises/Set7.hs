...
member x (Set []) = False
member x (Set (a:as)) = if x == a then True else member x (Set as)

-- add a member to a set
add :: Eq a => Ord a => a -> Set a -> Set a
add a (Set xs) = if member a (Set xs) == True then (Set xs) else (Set (sort (a:xs)))
...
reverseNonEmpty (x:|xs) = (head nl:| tail nl) where nl = reverse (x:xs)
...
passwordAllowed str (MinimumLength n) = if length str >= n then True else False

passwordAllowed [] (ContainsSome char) = False
passwordAllowed (s:xs) (ContainsSome char) = if elem s char == True then True else passwordAllowed xs (ContainsSome char)

passwordAllowed str (DoesNotContain char) = if passwordAllowed str (ContainsSome char) == True then False else True
...
operation opr x y = Operation opr x y

evaluate :: Arithmetic -> Integer
evaluate (Literal x) = x
evaluate (Operation opr x y)
  | opr == "+"  = (+) (evaluate x) (evaluate y)
  | opr == "*"  = (*) (evaluate x) (evaluate y)

render :: Arithmetic -> String
render (Literal x)         = show x
render (Operation opr x y) = "(" ++ (render x) ++ (opr) ++ (render y) ++ ")"
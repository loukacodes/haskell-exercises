...
getAmount :: ShoppingEntry -> Int
getAmount (MkShoppingEntry fruit price amount) = amount

getPrice :: ShoppingEntry -> Double
getPrice (MkShoppingEntry fruit price amount) = price

totalPrice :: ShoppingEntry -> Double
totalPrice entry = fromIntegral(getAmount entry) * getPrice entry
...
buyOneMore (MkShoppingEntry fruit price amount) = (MkShoppingEntry fruit price (amount + 1))
...
toggle (Increasing a) = (Decreasing a)
toggle (Decreasing a) = (Increasing a)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix col1 col2) = zipWith (\a b -> (a + b) / 2) (rgb col1) (rgb col2) 
